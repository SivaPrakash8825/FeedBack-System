...
const useReportGenerator = () => {
          ...
          data.forEach((val: any) => {
            ...
            if (!allfields[key]) {
              allfields[key] = {
                ...val,
                dept: val.dept,
                marks: [[val.username,val.stdtype,val.section,val.sem ,...parseanswer(val)]],
                usercomments: [[val.username, val.comments]],
              };
            } else {
              const usercom = [val.username, val.comments];

              allfields[key].marks.push([val.username,val.stdtype,val.section,val.sem, ...parseanswer(val)]);
    ...
    function ForTheoryAndLab(data: any): { header: any, newallfield: any, avgheader: any } {
        const allfields:any={}
        const header = [];
        const avgheader = [];
        header.push(
